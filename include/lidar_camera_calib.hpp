...
Calibration::Calibration(const std::string &image_file,
                         ...
                         const std::string &calib_config_file) 
                        : rclcpp::Node("lidar_camera_calib")
{
  // 初始化发布者
  rgb_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("rgb_cloud", 1);
  init_rgb_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("init_rgb_cloud", 1);
  planner_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("planner_cloud", 1);
  line_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("line_cloud", 1);
  image_pub_ = this->create_publisher<sensor_msgs::msg::Image>("camera_image", 1);
  ...
  if (!image_.data) {
    ...
    RCLCPP_ERROR_STREAM(this->get_logger(), msg.c_str());
    rclcpp::shutdown();
  } else {
    std::string msg = "Sucessfully load image!";
    RCLCPP_INFO_STREAM(this->get_logger(), msg.c_str());
  }
  width_ = image_.cols;
  height_ = image_.rows;
  // check rgb or gray
  ...
  RCLCPP_INFO_STREAM(this->get_logger(), "Loading point cloud from PCD file.");
...
bool Calibration::loadCalibConfig(const std::string &config_file) {
  ...
  if (!fSettings.isOpened()) {
    ...
    RCLCPP_INFO(this->get_logger(), "Successfully loaded calibration config file.");
  }
...
void Calibration::initVoxel(
    ...
    RCLCPP_INFO_STREAM(this->get_logger(), "Building Voxel");
    // for voxel test
...
void Calibration::LiDAREdgeExtraction(
      ...
      const float ransac_dis_thre, const int plane_size_threshold,
      pcl::PointCloud<pcl::PointXYZI>::Ptr &lidar_line_cloud_3d) {
    RCLCPP_INFO_STREAM(this->get_logger(), "Extracting Lidar Edge");
