...
void roughCalib(std::vector<std::shared_ptr<Calibration>> &calibs, Vector6d &calib_params,
...
int main(int argc, char **argv) {
  ...
  image_path = node->declare_parameter<std::string>("common.image_path", "");
  pcd_path = node->declare_parameter<std::string>("common.pcd_path", "");
  result_path = node->declare_parameter<std::string>("common.result_path", "");
  data_num = node->declare_parameter<int>("common.data_num", 1);
  camera_matrix = node->declare_parameter<std::vector<double>>("camera.camera_matrix", std::vector<double>{});
  dist_coeffs = node->declare_parameter<std::vector<double>>("camera.dist_coeffs", std::vector<double>{});
  use_rough_calib = node->declare_parameter<bool>("calib.use_rough_calib", false);
  calib_config_file = node->declare_parameter<std::string>("calib.calib_config_file", "");
  ...
  for (size_t i = 0; i < data_num; i++) {
    ...
    auto single_calib = std::make_shared<Calibration>(image_file, pcd_file, calib_config_file);
