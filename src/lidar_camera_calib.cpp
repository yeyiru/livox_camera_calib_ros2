...
void roughCalib(std::shared_ptr<Calibration> &calibra, Vector6d &calib_params,
...
int main(int argc, char **argv) {
  ...
  image_file = node->declare_parameter<std::string>("common.image_file", "");
  pcd_file = node->declare_parameter<std::string>("common.pcd_file", "");
  result_file = node->declare_parameter<std::string>("common.result_file", "");
  std::cout << "pcd_file path:" << pcd_file << std::endl;
  std::vector<double> camera_matrix = node->declare_parameter<std::vector<double>>("camera.camera_matrix", std::vector<double>());
  std::vector<double> dist_coeffs = node->declare_parameter<std::vector<double>>("camera.dist_coeffs", std::vector<double>());
  bool use_rough_calib = node->declare_parameter<bool>("calib.use_rough_calib", false);
  std::string calib_config_file = node->declare_parameter<std::string>("calib.calib_config_file", "");

  // Calibration calibra(image_file, pcd_file, calib_config_file);
  auto calibra = std::make_shared<Calibration>(image_file, pcd_file, calib_config_file);
